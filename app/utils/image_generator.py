...
def generate_template_preview_image(template_info: dict, resume_data: dict = None) -> bytes:
    ...
    # High-Visibility Ultra-Legible Typography for Maximum Clarity (900px canvas)
    ...
    if layout == "header_banner":
        ...
        draw.rectangle([0, 0, 26, max_canvas_height], fill=primary_rgb)
        ...
        draw.rectangle([0, 0, 290, max_canvas_height], fill="#F8FAFC")
        draw.line([(_i(290), 0), (_i(290), max_canvas_height)], fill=primary_rgb, width=_i(4))
    ...
    for proj in projects:
        ...
        draw_wrapped_text(proj_header, prefix="", font_to_use=item_title_font, text_color="#0F172A", indent=0, max_width_chars=26 if layout == "sidebar_column" else 42)
    ...
    # Footer Bar
    footer_y = curr_y + 16
    draw.rectangle([0, _i(footer_y), width, _i(footer_y + 44)], fill="#F8FAFC")
    draw.text((_i(45), _i(footer_y + 12)), f"NextHire AI Preview  |  Template: {template_info.get('name')}  |  Layout: {layout.replace('_', ' ').title()}", fill="#334155", font=small_font)

    total_h = max(1200, _i(footer_y + 44))

    if layout == "border_frame":
        draw.rectangle([_i(12), _i(12), _i(width - 12), _i(total_h - 12)], outline=primary_rgb, width=_i(4))
