...
def generate_template_preview_image(template_info: dict, resume_data: dict = None) -> bytes:
    ...
    # Complete A4 Overview Typography (600x848 Canvas)
    title_font = _load_font(36, font_type, bold=True)
    ...
    if layout == "header_banner":
        ...
        draw.text((_i(30), _i(16)), name, fill="#FFFFFF", font=title_font)
        draw.text((_i(30), _i(62)), f"{email}  |  {phone}  |  {location}", fill="#F1F5F9", font=subtitle_font)
        curr_y = 120
        ...
        draw.text((_i(36), _i(60)), f"{email}  |  {phone}  |  {location}", fill="#334155", font=subtitle_font)
        draw.line([(_i(36), _i(92)), (_i(width - 30), _i(92))], fill=primary_rgb, width=_i(3))
        curr_y = 106
        left_m = 36

    elif layout == "split_header":
        draw.text((_i(30), _i(14)), name, fill=primary_rgb, font=title_font)
        draw.text((_i(30), _i(58)), f"{email}  |  {phone}  |  {location}", fill="#1E293B", font=subtitle_font)
        draw.line([(_i(30), _i(90)), (_i(width - 30), _i(90))], fill=primary_rgb, width=_i(3))
        curr_y = 104

    elif layout == "centered_minimal":
        center_x = _i(width // 2)
        draw.text((_i(center_x - 160), _i(14)), name, fill=primary_rgb, font=title_font)
        draw.text((_i(center_x - 190), _i(58)), f"{email}   •   {phone}   •   {location}", fill="#334155", font=subtitle_font)
        draw.line([(_i(40), _i(90)), (_i(width - 40), _i(90))], fill=primary_rgb, width=_i(2))
        curr_y = 104

    elif layout == "code_terminal":
        draw.rectangle([0, 0, width, 105], fill="#1E1E2E")
        draw.text((_i(30), _i(14)), "$ cat candidate_profile.json", fill="#A6E3A1", font=subtitle_font)
        draw.text((_i(30), _i(42)), f"\"name\": \"{name}\", \"role\": \"AI Developer\"", fill="#F9E2AF", font=subtitle_font)
        draw.text((_i(30), _i(70)), f"\"contact\": \"{email} | {phone}\"", fill="#89B4FA", font=subtitle_font)
        curr_y = 120

    elif layout == "executive_serif":
        draw.line([(_i(30), _i(14)), (_i(width - 30), _i(14))], fill=primary_rgb, width=_i(3))
        draw.text((_i(30), _i(22)), name, fill=primary_rgb, font=title_font)
        draw.text((_i(30), _i(62)), f"EXECUTIVE PROFILE  |  {email}  |  {phone}", fill="#1E293B", font=subtitle_font)
        draw.line([(_i(30), _i(94)), (_i(width - 30), _i(94))], fill=primary_rgb, width=_i(3))
        curr_y = 108

    elif layout == "modern_pills":
        draw.rectangle([0, 0, width, 105], fill="#F8FAFC")
        draw.rectangle([_i(30), _i(14), _i(410), _i(58)], fill=primary_rgb)
        draw.text((_i(42), _i(18)), name, fill="#FFFFFF", font=title_font)
        draw.text((_i(30), _i(68)), f"{email}  |  {phone}  |  {location}", fill="#334155", font=subtitle_font)
        draw.line([(_i(30), _i(94)), (_i(width - 30), _i(94))], fill=primary_rgb, width=_i(3))
        curr_y = 108
        ...
        draw.text((_i(36), _i(16)), name, fill=primary_rgb, font=title_font)
        draw.text((_i(36), _i(60)), f"{email}  |  {phone}  |  {location}", fill="#334155", font=subtitle_font)
        draw.line([(_i(36), _i(90)), (_i(width - 36), _i(90))], fill=primary_rgb, width=_i(3))

    elif layout == "top_accent_bar":
        draw.rectangle([0, 0, width, 14], fill=primary_rgb)
        draw.rectangle([_i(30), _i(20), _i(width - 30), _i(94)], fill="#F8FAFC", outline="#CBD5E1", width=_i(1))
        draw.text((_i(42), _i(26)), name, fill=primary_rgb, font=title_font)
        draw.text((_i(42), _i(66)), f"{email}  |  {phone}  |  {location}", fill="#334155", font=subtitle_font)
        curr_y = 108

    elif layout == "sidebar_column":
        draw.rectangle([0, 0, 180, height], fill="#F8FAFC")
        draw.line([(_i(180), 0), (_i(180), height)], fill=primary_rgb, width=_i(3))
        draw.text((_i(12), _i(16)), name, fill=primary_rgb, font=_load_font(20, font_type, bold=True))
        draw.text((_i(12), _i(52)), f"{email}\n{phone}\n{location}", fill="#334155", font=subtitle_font)
        draw.text((_i(12), _i(130)), "SKILLS", fill=primary_rgb, font=section_font)
        draw.text((_i(12), _i(162)), "• Python & SQL\n• Streamlit & APIs\n• Gemini LLMs\n• Data Science", fill="#0F172A", font=body_font)
        left_m = 196
        right_m = 20
        curr_y = 18

    elif layout == "creative_gradient":
        draw.rectangle([0, 0, width, 56], fill=primary_rgb)
        draw.rectangle([0, 56, width, 90], fill="#38BDF8")
        draw.text((_i(30), _i(12)), name, fill="#FFFFFF", font=title_font)
        draw.text((_i(30), _i(62)), f"{email}  |  {phone}  |  {location}", fill="#FFFFFF", font=subtitle_font)
        curr_y = 104

    else:
        # Default layout fallback
        draw.rectangle([0, 0, width, 100], fill=primary_rgb)
        draw.text((_i(30), _i(14)), name, fill="#FFFFFF", font=title_font)
        draw.text((_i(30), _i(60)), f"{email}  |  {phone}  |  {location}", fill="#F1F5F9", font=subtitle_font)
        curr_y = 112
    ...
    def draw_section_header(title):
        ...
        if layout == "code_terminal":
            ...
            draw.rectangle([_i(left_m), _i(curr_y), _i(left_m + 280), _i(curr_y + 30)], fill=primary_rgb)
            draw.text((_i(left_m + 10), _i(curr_y + 2)), title, fill="#FFFFFF", font=section_font)
            curr_y += 34
        elif layout == "executive_serif":
    ...
    def draw_wrapped_text(text_str, prefix="", font_to_use=body_font, text_color="#0F172A", indent=0, max_width_chars=48):
        nonlocal curr_y
    ...
    draw_wrapped_text(summary, font_to_use=body_font, text_color="#0F172A", max_width_chars=30 if layout == "sidebar_column" else 48)
    curr_y += 8
    ...
    for exp in experience:
        ...
        if isinstance(bullets, list):
            for bullet in bullets:
                draw_wrapped_text(str(bullet), prefix="•  ", font_to_use=body_font, text_color="#0F172A", indent=12, max_width_chars=28 if layout == "sidebar_column" else 44)
        curr_y += 6
    ...
    for proj in projects:
        ...
        draw_wrapped_text(proj_header, prefix="", font_to_use=item_title_font, text_color="#0F172A", indent=0, max_width_chars=28 if layout == "sidebar_column" else 42)
    ...
    draw.text((_i(30), _i(footer_y + 8)), f"NextHire AI  |  Complete A4 Resume Overview  |  Template: {template_info.get('name')}", fill="#334155", font=small_font)
